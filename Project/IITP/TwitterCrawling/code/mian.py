...
crawled_data = crawled_data.reset_index().rename(columns={"index" : 'number'})
crawled_data.number = crawled_data.number.apply(lambda x : x+1)
crawled_data.to_csv('../Data/CleanedData/2101_covid_twitter.csv',index=False)
...
crawled_data = crawled_data.reset_index().rename(columns={"index" : 'number'})
crawled_data.number = crawled_data.number.apply(lambda x : x+1)
crawled_data.to_csv('../Data/CleanedData/2101_covid_twitter_removed.csv',index=False)
#%%
## 형태소 분석
def morpheme (crawled_data,crawledName):
    ...
    pd.DataFrame(count,index=[0]).to_csv('../Data/scrapedData/'+crawledName+'_twitter_count.csv.csv',index= False)
    return count
...
df.to_csv('../Data/scrapedData/hos_twitter_NonCleaned_morphemed.csv')
...
#%%
targetWords = readCount('../Data/process/hos_twitter_count_ver2.csv')
#%%
# 불용어 처리
stopwords = pd.read_csv('../Data/process/stopwords.csv')
...
mask = np.array(Image.open('../Data/img/Cough.png'))
...
plt.show()
fig.savefig('../Data/output/hos_wordCloud.png')
...
okt = Okt()
result = []
targetWords = cleanStr(pd.read_csv('../Data/scrapedData/hos_twitter_NonCleaned_morphemed.csv'))
...
count = Counter(result)
# count = count.most_common(500)
pd.DataFrame(count,index=[0]).to_csv('../Data/scrapedData/hos_twitter_count_ver2.csv',index= False)

####################  잠시 함수만들기 귀찮아서 이렇게 해봄 ########

targetWords = cleanStr(pd.read_csv('../Data/scrapedData/covid_twitter_NonCleaned_morphemed.csv'))
...
count = Counter(result)
# count = count.most_common(500)
pd.DataFrame(count,index=[0]).to_csv('../Data/process/covid_twitter_count_ver2.csv',index= False)
process